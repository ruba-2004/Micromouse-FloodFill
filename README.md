...
The navigation engine assumes an open grid and utilizes a Breadth-First Search (BFS) queue to calculate Manhattan distances from the target center. As the robot discovers walls via LiDAR (`wallDetectDist < 150mm`), the internal `mazeWalls` bitmask is updated, and the BFS queue dynamically recalculates the optimal escape route, preventing exhaustive dead-end searches.

### 2. Multi-Sensor PID Fusion
Straight-line movement without mechanical drift is maintained via the `fusedStraightCorrectionFull()` function, which dynamically weights three distinct error layers:
* **Layer 1 (Gyroscope):** Maintains absolute heading lock (`currentYaw - headingLock`).
* **Layer 2 (Encoders):** Synchronizes left and right wheel odometry to prevent slipping (`relLeft - relRight`).
* **Layer 3 (LiDAR):** Provides absolute physical centering between maze walls using dual-lateral measurements.

### 3. Finite State Machine (FSM)
To prevent blocking code and maintain high-frequency sensor polling, the main `loop()` is governed by a switch-case FSM. This dictates discrete operational states including `MAZE_SCAN`, `MAZE_DECIDE`, `MAZE_FORWARD`, and complex 3-phase kinematic turns (`TURN_LEFT_BACKUP` -> `TURN_LEFT_PIVOT` -> `TURN_LEFT_FORWARD`).
...
The system utilizes a dual-I2C bus architecture to prevent sensor polling conflicts. 
...
| **Right Encoder** | 34 (A), 35 (B) | Hardware Interrupts |
| **Motor L / R** | 13, 27 / 25, 26 | PWM Output |
...
2. **I2C Bus Saturation:** Running three ToF sensors and an IMU on the default ESP32 I2C pins resulted in bus lockups and continuous polling failures. **Resolution:** Shifted to a dual-I2C architecture utilizing the `<Wire.h>` library. The IMU operates independently on `Wire0`, while the LiDAR array operates on `Wire1`, utilizing `XSHUT` pins to assign sequential hex addresses (0x30, 0x31, 0x29) during boot.

---

## ⚙️ Dependencies & Installation
To compile this firmware, the following libraries are required:
* `Wire.h` / `WiFi.h` / `WebServer.h` (Native ESP32)
* `Adafruit_VL53L0X.h`
* `Adafruit_MPU6050.h`
* `Adafruit_Sensor.h`

Compile using the Arduino IDE or PlatformIO with the board set to **ESP32 Dev Module**. Ensure the baud rate for the Serial Monitor is set to `115200`.
